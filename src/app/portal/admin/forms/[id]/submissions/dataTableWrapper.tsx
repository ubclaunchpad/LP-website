...
import {createColumns, FormFields} from "@/app/portal/admin/forms/[id]/submissions/columns";
...
export  function getFormFields({users}: {users: any}): FormFields {
    return {
        ...
        role: {
            type: "select",
            label: "Which role are you applying for? - only the option you select will be considered",
            options: [
...
export default function DataTableWrapper<TData>({  data, users } : DataTableWrapperProps<TData>
) {
  ...
  return (
    <DataTable columns={columns} data={fieldData} fields={fields} />
  );
}