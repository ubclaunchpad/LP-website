...
function aggregateColumn(data: Record<string, any>[], column: string): AggregationResult {
    const aggData = data.reduce((acc: Record<string, AggregatedValue>, row: Record<string, any>) => {
...
export async function getFormAnalytics(formId: number, config: temp) {
  ...
  const [submissions, form]= await  Promise.all([getSubmissions(formId),getForm(formId)]);
      if (!form) {
     throw new Error("No form found");
        }
  const chartData = config.columns.map((column) => aggregateColumn(submissions, column));
  ...
  return {  analyticsData: chartData , stats: {
    submissions: submissions.length,
      lastUpdated: lastUpdated,}
  }
}