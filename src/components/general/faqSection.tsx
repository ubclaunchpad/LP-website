...
export default function FaqSection({ faqs }: FaqSectionProps) {
    ...
    const makeFAQColumn = useCallback((faqs: Faq[], start: number, end: number) => {
        return faqs.slice(start, end).map((faq, index) => (
            <QuestionAccordion
                key={index}
                faq={faq}
                index={index + start}
    ...
    return (
        <div className="flex flex-col items-center">
            <h2 className="text-3xl text-heading font-[600] text-[48px] text-white ">FAQs</h2>
...
function QuestionAccordion({faq, index, openIndex, setOpenIndex}: QuestionAccordionProps) {
    return (
        <div className="bg-[#3A3543] py-[20px] px-[20px] h-fit rounded-lg ">
            ...
            <dd className={`transition-all duration-300 px-2.5 ease-in-out text-white ${index === openIndex ? "max-h-fit py-10 " : "max-h-0  overflow-hidden"}`}>
