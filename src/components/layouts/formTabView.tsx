...
export const formContext = createContext({} as { rawForm: any, formFields: FormFields, submissions: any[] });
...
export default function FormTabView({ tabs, children, form }: TabViewProps) {
  ...
  return (
    ...
    <div
      className={
        "flex flex-col gap-4 flex-1 w-full min-h-screen  "
      }
    >
      <div className="flex justify-between items-center gap-2 border-background-500 border-b">
        <div className="flex flex-row p-2 items-center px-4 gap-2">
             <Link href={"/portal/admin/forms"} className={"  p-0.5 hover:border-lp-500 hover:text-lp-500 border  rounded-full border-transparent font-heading"}>
               <ArrowLeft size={18} />
             </Link>
            <h1 className="text-lg flex-1 flex-shrink-0 font-medium font-heading">{form.rawForm.title}</h1>
        </div>
